    ...
    while attempt <= max_retries:
        try:
            ...
            if response.status_code == 200:
                logging.debug(f"API request successful for CBX ID {cbx_id}: {response.status_code} OK")
                return True
            else:
...
def send_batch_api_updates(
    ...
    for profile in results:
        ...
        try:
            ...
            logging.debug(f"API updates posted for {player_name} ({cbx_id}) - {len(new_months)} months")

        except Exception as e:
            failed_count += len(new_months)
            logging.error(f"Error posting API update for {cbx_id}: {e}")
