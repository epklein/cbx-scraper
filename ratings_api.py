"""
External ratings API integration module for CBX Rating Scraper.
...
def _load_api_config() -> Optional[Dict[str, str]]:
    ...
    Environment variables:
        - CBX_RATINGS_API_ENDPOINT: Full URL to API endpoint (e.g., https://app.chesshub.cloud/api/cbx-ratings/)
        - API_TOKEN: Authentication token for the API
    """
    endpoint = os.getenv('CBX_RATINGS_API_ENDPOINT', '').strip()
    ...
    if endpoint and not token:
        logging.warning("CBX_RATINGS_API_ENDPOINT is set but API_TOKEN is missing - API posting disabled")
        return None

    if token and not endpoint:
        logging.warning("API_TOKEN is set but CBX_RATINGS_API_ENDPOINT is missing - API posting disabled")
        return None
    ...
    while attempt <= max_retries:
        try:
            ...
            if response.status_code == 200:
                logging.info(f"API request successful for CBX ID {cbx_id}: {response.status_code} OK")
                return True
            else:
                ...
                logging.error(f"API returned {response.status_code} for CBX ID {cbx_id}: {error_msg}")
            ...
            logging.error(f"API request timeout for CBX ID {cbx_id} after {timeout} seconds (attempt {attempt + 1}/{max_retries + 1})")
            ...
            logging.error(f"Failed to connect to API for CBX ID {cbx_id}: {str(e)} (attempt {attempt + 1}/{max_retries + 1})")
            if attempt < max_retries:
                attempt += 1
                continue
            return False
            ...
            logging.error(f"Unexpected error posting to API for CBX ID {cbx_id}: {str(e)}")
            return False
...
def send_batch_api_updates(
    ...
    for profile in results:
        ...
        cbx_id = profile.get('CBX ID', 'unknown')
        ...
        try:
            ...
            print(f"✓ API updates posted for {player_name} ({cbx_id}) - {len(new_months)} months", file=sys.stderr)

        except Exception as e:
            failed_count += len(new_months)
            print(f"✗ Error posting API update for {cbx_id}: {e}", file=sys.stderr)
