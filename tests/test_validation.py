"""
Unit tests for CBX ID and email validation functions.

Tests the validate_cbx_id() and validate_email() functions
...
from cbx_scraper import validate_cbx_id, validate_email


class TestValidateCbxId:
    """Test cases for validate_cbx_id() function."""

    def test_valid_cbx_ids(self):
        """Test that valid CBX IDs pass validation."""
        valid_ids = [
            "12345678",  # 8 digits
        ...
        for cbx_id in valid_ids:
            assert validate_cbx_id(cbx_id) is True, f"Expected {cbx_id} to be valid"
    ...
    def test_invalid_length(self):
        ...
        for cbx_id in invalid_ids:
            assert validate_cbx_id(cbx_id) is False, f"Expected {cbx_id} to be invalid"

    def test_non_numeric(self):
        """Test that non-numeric CBX IDs fail."""
        invalid_ids = [
    ...
    def test_none_and_non_string(self):
        ...
        for invalid_input in invalid_inputs:
            assert validate_cbx_id(invalid_input) is False, f"Expected {invalid_input} to be invalid"
...
class TestValidationIntegration:
    """Integration tests combining CBX ID and email validation."""

    def test_validate_player_data(self):
        """Test validating a player record with both CBX ID and email."""
        player_data = {
            "cbx_id": "12345678",
            "email": "player@example.com",
        }
        assert validate_cbx_id(player_data["cbx_id"]) is True
