"""
Integration tests for cbx_scraper.py
...
class TestEndToEndFlow:
    ...
    def test_end_to_end_with_valid_cbx_id(self):
        """Test complete flow with a known valid CBX ID."""
        # Using a well-known player ID (Magnus Carlsen: 94157)
        cbx_id = "94157"

        # Validate CBX ID
        assert cbx_scraper.validate_cbx_id(cbx_id) == True

        # Fetch profile
        html = cbx_scraper.fetch_cbx_profile(cbx_id)
        assert html is not None

        # Extract rating history
        history = cbx_scraper.extract_rating_history(html)

        # At least one month should be present
    ...
    def test_end_to_end_invalid_cbx_id(self):
        """Test complete flow with invalid CBX ID."""
        cbx_id = "invalid123"
        
        # Should fail validation
        assert cbx_scraper.validate_cbx_id(cbx_id) == False
    
    def test_end_to_end_nonexistent_cbx_id(self):
        """Test complete flow with non-existent CBX ID."""
...
class TestBatchProcessing:
    ...
    def test_end_to_end_batch_processing(self, tmp_path):
        """Test end-to-end batch processing with real CBX IDs."""
        # Create input CSV file with valid CBX IDs and emails
        input_file = tmp_path / "players.csv"
        input_file.write_text("CBX ID,email\n94157,player1@example.com\n2016892,player2@example.com\n")

        # Load player data from CSV
        player_data = cbx_scraper.load_player_data_from_csv(str(input_file))
        assert len(player_data) == 2

        # Extract CBX IDs from player data
        ...
        # Write CSV output
        output_file = tmp_path / "cbx_ratings.csv"
        cbx_scraper.write_csv_output(str(output_file), results)
        ...
        assert '94157' in content  # CBX ID present
        # Check that we have multiple date entries (monthly history)
        lines = content.strip().split('\n')
        assert len(lines) > 2  # Header + at least one data row

        # Verify console output
        console_output = cbx_scraper.format_console_output(results)
        assert 'Date' in console_output
        assert 'CBX ID' in console_output
    ...
    def test_batch_processing_mixed_valid_invalid(self, tmp_path):
        ...
        # Create input CSV file with multiple valid CBX IDs
        input_file = tmp_path / "players.csv"
        input_file.write_text("CBX ID,email\n94157,player1@example.com\n2016892,player2@example.com\n")
        ...
        assert "94157" in player_data
        assert "2016892" in player_data
        assert player_data["94157"]["email"] == "player1@example.com"
...
class TestEmailNotificationIntegration:
    ...
    def test_player_data_to_email_pipeline(self, tmp_path):
        ...
        # Load player data
        player_data = cbx_scraper.load_player_data_from_csv(str(players_file))
        assert len(player_data) == 2
        assert player_data["12345678"]["email"] == "alice@example.com"

        # Load historical ratings
        historical_data = cbx_scraper.load_historical_ratings_by_player(str(history_file))
        ...
        alice_new_months = cbx_scraper.detect_new_months(
            "12345678",
            alice_scraped_history,
            historical_data
        )
        ...
        bob_new_months = cbx_scraper.detect_new_months(
            "87654321",
            bob_scraped_history,
            historical_data
        )
    ...
    def test_batch_processing_with_change_detection(self, tmp_path):
        ...
        history_file = tmp_path / "history.csv"
        history_file.write_text("Date,CBX ID,Player Name,Standard,Rapid,Blitz\n")
        ...
        # Process batch
        results, errors = cbx_scraper.process_batch(cbx_ids, historical_data)
...
class TestFullEmailNotificationPipeline:
    ...
    def test_full_pipeline_load_process_send(self, mock_smtp_class, tmp_path):
        ...
        all_new_months = {}
        for cbx_id, scraped_history in scraped_histories.items():
            new_months = cbx_scraper.detect_new_months(
                cbx_id, scraped_history, historical_data
            )
            all_new_months[cbx_id] = new_months
        ...
        results = []
        for cbx_id, new_months in all_new_months.items():
            results.append({
                "CBX ID": cbx_id,
                "Player Name": player_names[cbx_id],
                "Rating History": scraped_histories[cbx_id],
...
@pytest.mark.integration
class TestCbxIdsApiIntegration:
    """Integration tests for CBX IDs API augmentation feature."""

    @patch('cbx_scraper.requests.get')
    def test_fetch_cbx_ids_api_success(self, mock_get):
        """Test successful API fetch with valid response."""
        # Mock successful API response
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "cbx_ids": ["12345678", "23456789", "34567890"],
            "count": 3
        }
        ...
        result = cbx_scraper.fetch_cbx_ids_from_api(
            "https://app.chesshub.cloud/api/cbx-ids/",
            "test_token"
        )
    ...
    @patch('cbx_scraper.requests.get')
    def test_fetch_cbx_ids_api_empty_response(self, mock_get):
        """Test API fetch with empty cbx_ids array."""
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "cbx_ids": [],
    ...
    @patch('cbx_scraper.requests.get')
    def test_fetch_cbx_ids_api_auth_error(self, mock_get):
    ...
    @patch('cbx_scraper.requests.get')
    def test_fetch_cbx_ids_api_timeout(self, mock_get):
    ...
    def test_merge_player_ids_no_duplicates(self):
        ...
        all_ids, new_ids = cbx_scraper.merge_player_ids(csv_ids, api_ids)
    ...
    def test_augment_players_file_creates_new_file(self, tmp_path):
        ...
        result = cbx_scraper.augment_players_file(str(csv_file), new_ids)
    ...
    def test_augment_players_file_empty_new_ids(self, tmp_path):
        ...
        result = cbx_scraper.augment_players_file(str(csv_file), [])
    ...
    def test_api_augmentation_full_flow(self, mock_get, tmp_path):
        ...
        mock_response.status_code = 200
        mock_response.json.return_value = {
            "cbx_ids": ["11111111", "22222222", "33333333"],
            "count": 3
        }
        mock_get.return_value = mock_response
        ...
        player_data = cbx_scraper.load_player_data_from_csv(str(csv_file))
...
class TestEdgeCases:
    ...
    def test_missing_api_configuration(self, mock_get, tmp_path):
        ...
        result = cbx_scraper.fetch_cbx_ids_from_api("", "test_token")
        assert result is None
        ...
        result = cbx_scraper.fetch_cbx_ids_from_api(None, None)
        assert result is None
    ...
    def test_malformed_csv_file_handling(self, tmp_path):
        ...
        # Test 1: CSV with missing CBX ID column
        ...
        with pytest.raises(ValueError, match="missing required headers"):
            cbx_scraper.load_player_data_from_csv(str(malformed_csv))

        # Test 2: CSV with invalid CBX IDs
        ...
        with pytest.raises(ValueError, match="CSV file is empty"):
            cbx_scraper.load_player_data_from_csv(str(empty_csv))
        ...
        player_data = cbx_scraper.load_player_data_from_csv(str(header_only_csv))
    ...
    def test_large_api_response_performance(self, mock_get, tmp_path):
        ...
        large_cbx_ids = [f"{2000000 + i}" for i in range(1000)]
        mock_response = MagicMock()
        mock_response.status_code = 200
        mock_response.json.return_value = {"cbx_ids": large_cbx_ids, "count": 1000}
        mock_get.return_value = mock_response
...
class TestQuickExecutionMode:
    ...
    @patch('cbx_scraper.fetch_cbx_ids_from_api')
    def test_quick_mode_selects_only_new_ids(self, mock_fetch):
        ...
        selected_ids, new_ids = cbx_scraper.select_cbx_ids_for_processing(
            mode='quick',
    ...
    @patch('cbx_scraper.fetch_cbx_ids_from_api')
    @patch('cbx_scraper.merge_player_ids')
    def test_normal_mode_processes_all_ids(self, mock_merge, mock_fetch):
        ...
        selected_ids, returned_new_ids = cbx_scraper.select_cbx_ids_for_processing(
            mode='normal',
            api_endpoint='http://api.example.com',
            api_token='test_token',
            csv_player_data=csv_player_data
