...
# Add parent directory to path to import cbx_scraper
...
class TestCbxIdValidation:
    """Tests for CBX ID validation."""
    
    def test_valid_cbx_id_numeric(self):
        """Test that valid numeric CBX IDs pass validation."""
        assert cbx_scraper.validate_cbx_id("94157") == True
        assert cbx_scraper.validate_cbx_id("123456") == True
        assert cbx_scraper.validate_cbx_id("12345678") == True
    
    def test_invalid_cbx_id_non_numeric(self):
        """Test that non-numeric CBX IDs fail validation."""
        assert cbx_scraper.validate_cbx_id("abc123") == False
    ...
    def test_invalid_cbx_id_too_long(self):
        ...
        assert cbx_scraper.validate_cbx_id("123456789012") == False
    
    def test_invalid_cbx_id_empty(self):
...
class TestErrorHandling:
    ...
    @patch('cbx_scraper.requests.get')
    ...
    def test_http_error_404(self, mock_get):
        ...
        result = cbx_scraper.fetch_cbx_profile("99999999")
...
class TestPlayerNameExtraction:
    ...
    def test_extract_player_name_primary_method(self):
        """Test extracting player name from div#dados-jogador-row1 > h2 (primary method)."""
        html = """
        <html>
            <head><title>CBX - Jogador: 94157 - Eduardo Pavinato Klein</title></head>
            <body>
                <div id="dados-jogador-row1">
                    <h2>Eduardo Pavinato Klein</h2>
                    <div>Other info</div>
                </div>
            </body>
        </html>
        """
        name = cbx_scraper.extract_player_name(html)
        assert name == "Eduardo Pavinato Klein"

    def test_extract_player_name_fallback_title_cbx_format(self):
        """Test extracting name from title tag with CBX format (fallback)."""
        html = """
        <html>
            <head><title>CBX - Jogador: 94157 - Eduardo Pavinato Klein</title></head>
            <body>
                <div>Some content without dados-jogador-row1</div>
            </body>
        </html>
        """
        name = cbx_scraper.extract_player_name(html)
    ...
    def test_extract_player_name_title_multiple_dashes(self):
        """Test extracting name from title with multiple dashes in player name."""
        html = """
        <html>
            <head><title>CBX - Jogador: 123456 - José María García-López</title></head>
            <body>
                <div>No h2 tag available</div>
            </body>
        </html>
        """
        name = cbx_scraper.extract_player_name(html)
        assert name == "José María García-López"

    def test_extract_player_name_missing_h2_in_div(self):
        """Test handling when div#dados-jogador-row1 exists but h2 is missing."""
        html = """
        <html>
            <head><title>CBX - Jogador: 123456 - Fallback Name</title></head>
            <body>
                <div id="dados-jogador-row1">
                    <p>No h2 tag here</p>
                </div>
        ...
        """
        name = cbx_scraper.extract_player_name(html)
        # Should fall back to title parsing
        assert name == "Fallback Name"

    def test_extract_player_name_empty_h2_tag(self):
        """Test handling when h2 tag exists but is empty."""
        html = """
        <html>
            <head><title>CBX - Jogador: 123456 - Title Name</title></head>
            <body>
                <div id="dados-jogador-row1">
                    <h2></h2>
                </div>
        ...
        """
        name = cbx_scraper.extract_player_name(html)
        # Should fall back to title parsing when h2 is empty
        assert name == "Title Name"

    def test_extract_player_name_title_two_parts(self):
        """Test title parsing with only two parts (edge case)."""
        html = """
        <html>
            <head><title>CBX - Fabiano Caruana</title></head>
            <body>
                <div>No other name source</div>
            </body>
        </html>
        """
        name = cbx_scraper.extract_player_name(html)
    ...
    def test_extract_player_name_title_jogador_prefix(self):
        """Test title parsing skips 'Jogador:' prefix in two-part format."""
        html = """
        <html>
            <head><title>CBX - Jogador: 123456</title></head>
            <body>
                <div>No other name source</div>
            </body>
        </html>
        """
        name = cbx_scraper.extract_player_name(html)
        # Should not return "Jogador: 123456" as it starts with "Jogador:"
        assert name is None
    ...
    def test_extract_player_name_none_html(self):
        ...
        name = cbx_scraper.extract_player_name(None)
        assert name is None
    ...
    def test_extract_player_name_whitespace_handling(self):
        """Test that whitespace is properly stripped from extracted names."""
        html = """
        <html>
            <body>
                <div id="dados-jogador-row1">
    ...
    def test_extract_player_name_special_characters(self):
        """Test extracting names with special characters (accents, hyphens)."""
        html = """
        <html>
            <body>
                <div id="dados-jogador-row1">
                    <h2>José García-López Pérez</h2>
                </div>
            </body>
        </html>
        """
        name = cbx_scraper.extract_player_name(html)
        assert name == "José García-López Pérez"

    def test_extract_player_name_all_methods_fail(self):
        """Test handling when all extraction methods fail."""
        html = """
        <html>
            <head><title>Page Title</title></head>
            <body>
                <div>No name information anywhere</div>
            </body>
        </html>
        """
        name = cbx_scraper.extract_player_name(html)
...
class TestConsoleOutputFormatting:
    ...
    def test_format_console_output_tabular_format(self):
        ...
        output = cbx_scraper.format_console_output(player_profiles)
        assert 'Date' in output
        assert 'CBX ID' in output
        assert 'Player Name' in output
        assert 'Standard' in output
        assert 'Rapid' in output
        assert 'Blitz' in output
        assert '94157' in output
    ...
    def test_format_console_output_column_alignment(self):
        ...
        lines = output.split('\n')
        data_lines = [line for line in lines if '94157' in line]
        if data_lines:
...
class TestLoadPlayerDataFromCSV:
    ...
    def test_load_player_data_from_csv_valid(self, tmp_path):
        ...
        result = cbx_scraper.load_player_data_from_csv(str(test_file))
    ...
    def test_load_player_data_from_csv_file_not_found(self):
        ...
        with pytest.raises(FileNotFoundError):
            cbx_scraper.load_player_data_from_csv("/nonexistent/path/players.csv")
    ...
    def test_load_player_data_from_csv_invalid_headers(self, tmp_path):
        ...
        with pytest.raises(ValueError) as exc_info:
            cbx_scraper.load_player_data_from_csv(str(test_file))
        assert "missing required headers" in str(exc_info.value).lower()

    def test_load_player_data_from_csv_invalid_cbx_id(self, tmp_path, capsys):
        """Test that invalid CBX IDs are skipped with warnings."""
        test_file = tmp_path / "players.csv"
        test_file.write_text(
            "CBX ID,email\n"
            "12345678,alice@example.com\n"
        ...
        # Check that warning was printed to stderr
        captured = capsys.readouterr()
        assert "Warning" in captured.err or "invalid cbx id" in captured.err.lower()
    ...
    def test_load_player_data_from_csv_whitespace_stripped(self, tmp_path):
        """Test that whitespace is stripped from CBX ID and email."""
        test_file = tmp_path / "players.csv"
        test_file.write_text(
            "CBX ID,email\n"
            "  12345678  ,  alice@example.com  \n"
...
class TestLoadHistoricalRatingsByPlayer:
    ...
    def test_load_historical_ratings_valid(self, tmp_path):
        ...
        test_file.write_text(
            ...
            "2025-11-01,12345678,Alice Smith,2440,2300,2100\n"
            "2025-11-01,87654321,Bob Jones,2500,2400,\n"
            "2025-10-01,12345678,Alice Smith,2450,2310,2110\n"
        )
    ...
    def test_load_historical_ratings_file_not_found(self):
        ...
        result = cbx_scraper.load_historical_ratings_by_player("/nonexistent/path/cbx_ratings.csv")
        assert result == {}
    ...
    def test_load_historical_ratings_unrated_handling(self, tmp_path):
        ...
        test_file.write_text(
            "Date,CBX ID,Player Name,Standard,Rapid,Blitz\n"
            "2025-11-01,12345678,Alice Smith,2440,,2100\n"
            "2025-11-01,87654321,Bob Jones,,,\n"
        )
        result = cbx_scraper.load_historical_ratings_by_player(str(test_file))
    ...
    def test_load_historical_ratings_converts_empty_to_none(self, tmp_path):
        ...
        test_file.write_text(
            "Date,CBX ID,Player Name,Standard,Rapid,Blitz\n"
            "2025-11-01,12345678,Alice Smith,2440,2300,\n"
        )
        result = cbx_scraper.load_historical_ratings_by_player(str(test_file))
...
class TestComposeNotificationEmail:
    ...
    def test_compose_notification_email_single_change(self):
        ...
        assert "CBX ID: 12345678" in body
        assert "CBX Rating Monitor" in body
    ...
    def test_compose_notification_email_multiple_changes(self):
        ...
        assert "CBX ID: 87654321" in body
    ...
    def test_compose_notification_email_without_cc_parameter(self):
        ...
        assert "Your CBX ratings have been updated" in body
...
class TestLoadApiConfig:
    ...
    @patch.dict(os.environ, {'CBX_RATINGS_API_ENDPOINT': 'https://api.example.com/ratings/', 'API_TOKEN': 'test-token-123'})
    ...
    def test_load_api_config_missing_both(self):
        ...
        for var in ['CBX_RATINGS_API_ENDPOINT', 'API_TOKEN']:
    ...
    @patch.dict(os.environ, {'CBX_RATINGS_API_ENDPOINT': 'https://api.example.com/ratings/', 'API_TOKEN': ''})
    def test_load_api_config_missing_token(self):
        """Test loading API configuration when token is missing."""
        config = ratings_api._load_api_config()
        assert config is None

    @patch.dict(os.environ, {'CBX_RATINGS_API_ENDPOINT': '', 'API_TOKEN': 'test-token-123'})
...
class TestPostRatingToApi:
    ...
    def test_post_rating_to_api_success(self, mock_post):
        ...
        assert call_kwargs['json']['cbx_id'] == '12345678'
