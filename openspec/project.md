...
CBX Scraper is a Python-based batch processing tool that automatically retrieves and tracks chess player ratings from the CBX (Confederação Brasileira de Xadrez) website. The project maintains historical rating records, sends email notifications when player ratings change, and integrates with external APIs for fetching player IDs and posting rating updates. It's designed for headless/scheduled execution via cron jobs and containerized deployment.
...
- **Modular File Structure**: Separate concerns into focused modules:
  - `cbx_scraper.py`: Core scraping logic, CSV I/O, orchestration
...
**Chess Rating System**: CBX (Brazilian Chess Federation) maintains official chess ratings in three time controls:
...
**Player Identification**: Players are identified by a 1-10 digit CBX ID (numeric). The scraper validates IDs before attempting to fetch profiles.
...
**Integration Points**:
- External API for fetching additional player CBX IDs (optional, with token auth)
...
- **Scraping Rate**: The CBX website should not be scrapped faster than once per hour per player ID (respectful scraping to avoid overloading their servers)
...
- **CBX Website** (`cbx.org.br`): Source of truth for player ratings; scraped via HTTP requests with BeautifulSoup HTML parsing
...
- **External API: Player IDs** (optional, configurable via `CBX_IDS_API_ENDPOINT`):
  - Provides list of additional player CBX IDs to scrape
...
- **External API: Rating Updates** (optional, configurable via `CBX_RATINGS_API_ENDPOINT`):
  - Receives POST requests with scraped rating data
  - Requires token authentication via `API_TOKEN`
  - Expected format: JSON with player ratings and timestamps
  - Fallback: If unavailable, ratings still saved to local `cbx_ratings.csv`
