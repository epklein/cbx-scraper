...
Implemented comprehensive rating history extraction from CBX profiles:

1. **Month Parsing Utilities** (`cbx_scraper.py` lines 89-139)
...
2. **Month/Year String Parsing** (`cbx_scraper.py` lines 142-182)
   - `_parse_month_year_string()`: Converts "Nov/2025" format to ISO 8601 dates (2025-11-30)

3. **Table Row Extraction** (`cbx_scraper.py` lines 230-321)
   - `_extract_all_history_rows()`: Extracts all rows from CBX history table (not just first row)
   - Handles all 4 columns: month/year, standard, rapid, blitz
   - Gracefully handles unrated values and malformed data

4. **Deduplication** (`cbx_scraper.py` lines 324-360)
   - `_deduplicate_history_by_month()`: Removes duplicate months, keeping topmost (most recent)

5. **History Conversion** (`cbx_scraper.py` lines 363-412)
...
6. **Public API** (`cbx_scraper.py` lines 415-447)
...
1. **CSV Output** (`cbx_scraper.py` lines 1027-1096)
...
2. **Historical Data Loading** (`cbx_scraper.py` lines 894-966)
   - Updated `load_historical_ratings_by_player()` to return lists of all monthly records
   - Format: `{cbx_id: [{"Date": "...", "Standard": ..., ...}, ...]}`
...
1. **New Month Detection** (`cbx_scraper.py` lines 969-1034)
...
2. **Process Batch Integration** (`cbx_scraper.py` lines 1150-1236)
...
- When a month appears multiple times in CBX table, keep the topmost entry
- Topmost entry is most recent in the table
- Ensures data consistency with CBX source of truth
...
1. **cbx_scraper.py** (core implementation)
...
3. Manual testing with real CBX profiles
...
# Verify syntax
python3 -m py_compile cbx_scraper.py email_notifier.py ratings_api.py
