...
- [ ] Rename/refactor `_extract_rating_from_cbx_table()` function to `_extract_all_history_rows(html: str) -> List[Dict]`
...
- [ ] Add integration tests
  - Test with real CBX HTML samples
...
**Acceptance Criteria:**
- CSV headers are correct: Month,CBX ID,Player Name,Standard,Rapid,Blitz
...
- [ ] Rename/refactor `load_historical_ratings_by_player()` to `load_historical_ratings_by_player_monthly()`
  ...
  - Build dict: `{cbx_id: [{"date": date_obj, "standard": int|None, ...}, ...]}`
...
- [ ] Create `detect_new_months(cbx_id: str, scraped_history: List[Dict], stored_history: Dict) -> List[Dict]` function
  - Input: scraped_history from Task 1.5, stored_history from Task 2.2
  - Extract month dates from stored_history[cbx_id] (or empty list if not found)
...
- [ ] Create `_compose_new_months_email(player_name: str, cbx_id: str, new_months: List[Dict], ...)`
...
- [ ] Review `test_cbx_scraper.py` and update tests
...
- [ ] Test with real CBX data (if possible)
