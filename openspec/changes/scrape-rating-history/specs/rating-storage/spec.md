...
Then the columns should be: `Date,CBX ID,Player Name,Standard,Rapid,Blitz` (unchanged from previous format)
...
When writing CSV output, the system SHALL replace any existing row with the same (CBX ID, Month) combination, preserving all other records.
