...
Extract complete rating history (all monthly records) from a CBX player profile instead of just the most recent record.

## ADDED Requirements

### Requirement: Extract All History Rows from Table

The system SHALL extract all data rows from the CBX rating history table (`ContentPlaceHolder1_gdvRating`), not just the first row. Each row represents one monthly record with month/year and three rating categories (Standard, Rapid, Blitz).

**ID:** `RHE-001`
**Priority:** High

#### Scenario: Player with multiple months of history
Given a CBX profile HTML with a rating history table containing 7 rows (Mar/2025 through Nov/2025)
...
Given a CBX profile with only one month in the rating table
When we extract rating history
...
Given a CBX profile where June/2025 appears twice in the table (at positions 3 and 5)
...
Given a CBX profile where all months are unique
When we deduplicate
...
The system SHALL convert Portuguese month/year strings (e.g., "Nov/2025", "Out/2025") to ISO 8601 date format using the first day of the month, because CBX creates new rating records at the beginning of each month.
...
#### Scenario: February
Given the month/year string "Fev/2024"
When we parse the date
...
Then each record should contain:
  - `date`: ISO 8601 date string (first day of month, e.g., 2025-11-01)
...
- Uses `date(year, month, 1)` to return the first day of the month
