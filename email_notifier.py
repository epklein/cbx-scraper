"""
Email notification module for CBX Rating Scraper.
...
def _compose_notification_email(
    ...
    cbx_profile_url: Optional[str] = None
) -> Tuple[str, str]:
    """
    Compose a notification email about rating changes.

    Generates an email subject and body informing a player about their CBX rating updates.
    The email includes their name, CBX ID, and details of changed ratings between the two
    most recent entries in their rating history.

    Args:
        player_name: Player's full name (e.g., "Alice Smith")
        cbx_id: Player's CBX ID (e.g., "12345678")
        ...
        cbx_profile_url: Optional full URL to CBX profile. If not provided, will be constructed from cbx_id.
    ...
    """
    # Construct CBX profile URL if not provided
    if cbx_profile_url is None:
        cbx_profile_url = f"https://www.cbx.org.br/jogador/{cbx_id}"

    # Compose subject
    subject = f"Your CBX Rating Update - {player_name}"
    ...
    # Add footer with CBX profile URL
    lines.extend([
        "",
        "CBX ID: " + cbx_id,
        "Profile: " + cbx_profile_url,
        "",
        "Best regards,",
        "CBX Rating Monitor",
...
def _send_email_notification(
    ...
    Examples:
        success = _send_email_notification(
            "alice@example.com",
            "admin@example.com",
            "Your CBX Rating Update - Alice Smith",
    ...
    try:
        ...
        msg['Subject'] = subject
        msg['From'] = smtp_username if smtp_username else 'noreply@cbx-monitor.local'
        msg['To'] = recipient
...
def send_batch_notifications(
    ...
    Args:
        ...
        player_data: Dictionary of player data with emails {cbx_id: {"email": "..."}, ...}
    ...
    for result in results:
        ...
        # Get player email
        if cbx_id not in player_data:
            continue

        player_email = player_data[cbx_id].get('email', '').strip()
        ...
        try:
            ...
            failed_count += 1
            print(f"✗ Error sending email to {cbx_id}: {e}", file=sys.stderr)
