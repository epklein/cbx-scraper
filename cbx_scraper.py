...
A simple script to retrieve chess player ratings from the CBX website.
"""
...
# Player data file (unified CSV with CBX IDs and emails)
CBX_PLAYERS_FILE = os.getenv('CBX_PLAYERS_FILE', 'players.csv')
# Output ratings file (historical ratings)
OUTPUT_FILENAME = os.getenv('CBX_OUTPUT_FILE', 'cbx_ratings.csv')

def validate_cbx_id(cbx_id: str) -> bool:
    """
    Validate CBX ID format.

    Args:
        cbx_id: The CBX ID to validate
...
def construct_cbx_url(cbx_id: str) -> str:
    ...
    Args:
        cbx_id: Validated CBX ID

    Returns:
        URL string for the CBX profile page
    """
    return f"https://www.cbx.org.br/jogador/{cbx_id}"


def fetch_cbx_profile(cbx_id: str, timeout: int = 10) -> Optional[str]:
    """
    Fetch CBX profile HTML page.
    ...
    try:
        ...
        raise ConnectionError(f"Unable to connect to CBX website: {e}")
    except requests.Timeout:
        raise requests.Timeout("Request to CBX website timed out")
...
def _extract_all_history_rows(html: str) -> List[Dict]:
    ...
    try:
        ...
        if not table:
            logger.debug("Rating table 'ContentPlaceHolder1_gdvRating' not found in page")
            return []
...
def extract_player_name(html: str) -> Optional[str]:
    """
    Extract player name from CBX profile HTML.

    Tries multiple fallback methods to extract the player name.
    Primary method: Look for h2 tag within div with id="dados-jogador-row1"
    Fallback: Parse the title tag for CBX format
    ...
    try:
        ...
        # Primary: Look for h2 tag within div#dados-jogador-row1
        dados_div = soup.find('div', {'id': 'dados-jogador-row1'})
        if dados_div:
            h2_tag = dados_div.find('h2')
            if h2_tag:
                name = h2_tag.get_text(strip=True)
                if name:
                    return name

        # Fallback 3: Parse title tag (CBX format: "CBX - Jogador: {id} - {name}")
        if soup.title:
            title_text = soup.title.get_text(strip=True)
            # Try to extract name from CBX format: "CBX - Jogador: 94157 - Eduardo Pavinato Klein"
            if ' - ' in title_text:
                parts = title_text.split(' - ')
                if len(parts) >= 3:
                    # Last part is the player name
                    name = parts[-1].strip()
                    if name:
                        return name
                elif len(parts) == 2:
                    # Fallback: just use the second part
                    name = parts[1].strip()
                    if name and not name.startswith('Jogador:'):
                        return name
...
def load_player_data_from_csv(filepath: str) -> Dict[str, Dict[str, str]]:
    """
    Load player data from CSV file with CBX IDs and optional emails.

    Parses a CSV file with headers "CBX ID" and "email", validates each row,
    and returns a dictionary indexed by CBX ID. Invalid entries are skipped
    ...
    Returns:
        Dictionary mapping CBX ID to player data:
        {cbx_id: {"email": "..."}, ...}
    ...
    try:
        with open(filepath, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            for row_num, row in enumerate(reader, start=2):  # Start at 2 (skip header)
                cbx_id = row.get('CBX ID', '').strip() if row.get('CBX ID') is not None else ''
                email = row.get('email', '').strip() if row.get('email') is not None else ''

                # Validate CBX ID
                if not validate_cbx_id(cbx_id):
                    logger.warning(f"Line {row_num} - Invalid CBX ID '{cbx_id}' (skipped)")
                    continue

                # Validate email (empty is ok, but must be valid if provided)
                if not validate_email(email):
                    logger.warning(f"Line {row_num} - Invalid email '{email}' for CBX ID {cbx_id} (skipped)")
                    continue

                # Add to player data
                player_data[cbx_id] = {"email": email}
...
def fetch_cbx_ids_from_api(api_endpoint: str, api_token: str) -> Optional[List[str]]:
    """
    Fetch CBX IDs from external API endpoint.

    Sends a GET request to the configured API endpoint with token authentication.
    Parses the JSON response and extracts the cbx_ids array as strings.
    ...
    Returns:
        List of CBX ID strings if successful, None if API unavailable/fails
    ...
    if not api_endpoint or not api_endpoint.strip():
        logging.info("CBX_IDS_API_ENDPOINT not configured")
        return None

    if not api_token or not api_token.strip():
        logging.info("API_TOKEN not configured for CBX IDs API")
        return None
    ...
    try:
        ...
        logging.info(f"Fetching CBX IDs from API: {api_endpoint}")
        ...
        if 'cbx_ids' not in data:
            logging.error("API response missing 'cbx_ids' field")
            return None

        cbx_ids_data = data.get('cbx_ids')
        if not isinstance(cbx_ids_data, list):
            logging.error(f"API response 'cbx_ids' is not an array: {type(cbx_ids_data)}")
            return None

        # Validate that all IDs are strings
        cbx_ids = []
        for cbx_id in cbx_ids_data:
            if isinstance(cbx_id, str):
                cbx_ids.append(cbx_id)
            else:
                logging.warning(f"API response contains non-string CBX ID: {cbx_id} (type: {type(cbx_id).__name__})")

        # Log success
        api_count = len(data.get('cbx_ids', []))
        valid_count = len(cbx_ids)
        logging.info(f"Successfully fetched {api_count} CBX IDs from API ({valid_count} valid strings)")

        return cbx_ids if cbx_ids else None
        ...
        logging.error(f"Unexpected error fetching CBX IDs from API: {e}")
        return None


def merge_player_ids(csv_ids: List[str], api_ids: List[str]) -> Tuple[List[str], List[str]]:
    """
    Merge CBX IDs from CSV and API, with deduplication.
    ...
    Args:
        csv_ids: List of CBX IDs from existing CSV file
        api_ids: List of CBX IDs from API response
...
def select_cbx_ids_for_processing(
    mode: str,
    api_endpoint: str,
    api_token: str,
    ...
    """
    Select CBX IDs for processing based on execution mode.
    ...
    logging.info(f"Current CSV contains {len(csv_ids)} CBX IDs")

    # Fetch API IDs if configured
    api_ids = None
    if api_endpoint and api_token:
        logging.info("Fetching CBX IDs from API")
        api_ids = fetch_cbx_ids_from_api(api_endpoint, api_token)
        if api_ids:
            ...
            logging.warning("Failed to fetch CBX IDs from API")
    else:
        logging.info("CBX IDs API is not configured (optional feature)")
...
def augment_players_file(csv_path: str, new_ids: List[str]) -> bool:
    """
    Append new CBX IDs to existing CSV file.
    ...
    Args:
        csv_path: Path to CBX_PLAYERS_FILE CSV file
    ...
    try:
        ...
        try:
            with open(csv_path, 'w', newline='', encoding='utf-8') as csvfile:
                ...
                for cbx_id, player_data in existing_players.items():
                    writer.writerow({'CBX ID': cbx_id, 'email': player_data.get('email', '')})
...
def load_historical_ratings_by_player(filepath: str) -> Dict[str, List[Dict]]:
    ...
    try:
        with open(filepath, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            required_fields = {'Date', 'CBX ID', 'Player Name', 'Standard', 'Rapid', 'Blitz'}
            if not required_fields.issubset(set(reader.fieldnames)):
...
def format_console_output(player_profiles: List[Dict]) -> str:
    ...
    # Header
    header = f"Date         CBX ID       Player Name                              Standard  Rapid  Blitz"
    ...
    for profile in player_profiles:
        ...
        row = f"{date_str:<12} {cbx_id:<12} {player_name:<40} {str(standard):<9} {str(rapid):<6} {str(blitz)}"
        lines.append(row)
...
def process_batch(cbx_ids: List[str], historical_data: Dict[str, List[Dict]] = None) -> Tuple[List[Dict], List[str]]:
    ...
    for cbx_id in cbx_ids:
        # Validate CBX ID format
        if not validate_cbx_id(cbx_id):
            msg = f"Invalid CBX ID format: {cbx_id} (skipped)"
            logger.warning(msg)
            errors.append(msg)
            continue
        ...
        try:
            # Fetch profile
            html = fetch_cbx_profile(cbx_id)

            if html is None:
                msg = f"Player not found (CBX ID: {cbx_id}) (skipped)"
                logger.warning(msg)
                errors.append(msg)
                continue
            ...
            if not rating_history and not player_name:
                ...
                has_table = 'ContentPlaceHolder1_gdvRating' in html
                snippet = re.sub(r'\s+', ' ', html[:300]).strip()
                msg = (
                    f"Unable to extract data from CBX profile (CBX ID: {cbx_id}) (skipped) "
                    f"[html_len={len(html)}, rating_table_present={has_table}]"
                )
                logger.warning(msg)
                logger.debug(f"CBX ID {cbx_id} page starts: {snippet}")
                errors.append(msg)
                continue
            ...
            msg = f"Network error for CBX ID {cbx_id}: {e} (skipped)"
            logger.error(msg)
            errors.append(msg)
            continue
        except requests.Timeout:
            msg = f"Request timeout for CBX ID {cbx_id} (skipped)"
            logger.error(msg)
            errors.append(msg)
            continue
        except requests.HTTPError as e:
            msg = f"HTTP error for CBX ID {cbx_id}: {e} (skipped)"
            logger.error(msg)
            errors.append(msg)
            continue
        except Exception as e:
            msg = f"Unexpected error for CBX ID {cbx_id}: {e} (skipped)"
            logger.error(msg)
            errors.append(msg)
            continue
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='CBX Rating Scraper - Batch process chess player ratings from CBX website',
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Execution Modes:

  Normal Mode (default):
    Processes all CBX IDs from players.csv file (including any new IDs from API).
    $ python cbx_scraper.py

  Quick Mode (--quick):
    Processes only new CBX IDs returned by the API (not in players.csv).
    Useful for incremental runs focused on recently added players.
    $ python cbx_scraper.py --quick

Configuration:
  Reads CBX IDs from the file specified in FIDE_PLAYERS_FILE environment variable
  ...
  See .env.example for configuration options.
    ...
    parser.add_argument(
        '--quick',
        action='store_true',
        help='Quick mode: process only new CBX IDs from API (skip existing players.csv IDs)'
    )
    ...
    try:
        ...
        logger.info(f"CBX scraper started - mode: {execution_mode.upper()}, time: {start_time.strftime('%Y-%m-%d %H:%M:%S')}")

        # Load player data from CSV file (includes CBX IDs and emails)
        logger.info(f"Loading player data from {CBX_PLAYERS_FILE}")
        t0 = datetime.now()
        try:
            player_data = load_player_data_from_csv(CBX_PLAYERS_FILE)
        except FileNotFoundError:
        ...
        api_endpoint = os.getenv('CBX_IDS_API_ENDPOINT', '').strip()
        api_token = os.getenv('API_TOKEN', '').strip()

        # Select CBX IDs based on execution mode
        logger.info("Selecting CBX IDs for processing")
        t0 = datetime.now()
        cbx_ids, new_ids = select_cbx_ids_for_processing(
            execution_mode,
            api_endpoint,
            api_token,
            player_data
        )
        logger.info(f"CBX ID selection done: {len(cbx_ids)} selected, {len(new_ids)} new ({(datetime.now() - t0).total_seconds():.1f}s)")

        # Augment players file with new IDs if any were discovered
        if new_ids:
            success = augment_players_file(CBX_PLAYERS_FILE, new_ids)
            if success:
                logging.info(f"Players file successfully augmented with {len(new_ids)} new CBX IDs")
        ...
        if not cbx_ids:
            if execution_mode == 'quick':
                ...
                logger.info("Quick mode: no new CBX IDs found from API")
                sys.exit(0)
            else:
        ...
        logger.info(f"[{execution_mode.upper()}] Scraping CBX ratings for {len(cbx_ids)} players")
        ...
        logger.debug("Latest CBX Ratings: \n" + console_output)
