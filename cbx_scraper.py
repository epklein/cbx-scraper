        ...
        with open(filepath, 'r', newline='', encoding='utf-8') as csvfile:
            ...
            for row_num, row in enumerate(reader, start=2):  # Start at 2 (skip header)
                ...
                if not validate_cbx_id(cbx_id):
                    print(
                        f"Warning: Line {row_num} - Invalid CBX ID '{cbx_id}' (skipped)",
                        file=sys.stderr
                    )
                    continue
                ...
                if not validate_email(email):
                    print(
                        f"Warning: Line {row_num} - Invalid email '{email}' for CBX ID {cbx_id} (skipped)",
                        file=sys.stderr
                    )
                    continue
...
Batch Processing Mode:
  Reads CBX IDs from the file specified in CBX_PLAYERS_FILE environment variable
  (default: players.csv) and writes results to CBX_OUTPUT_FILE (default: cbx_ratings.csv).
  ...
  See .env.example for configuration options.
    ...
    try:
        ...
        # Augment players file with CBX IDs from API if configured
        api_endpoint = os.getenv('CBX_IDS_API_ENDPOINT', '').strip()
        api_token = os.getenv('API_TOKEN', '').strip()

        if api_endpoint and api_token:
            logging.info("CBX IDs API is configured, attempting to fetch and augment players file")
            ...
            if api_ids:
                ...
                if success:
                    logging.info(f"Players file successfully augmented with {len(new_ids)} new CBX IDs from API")
                    # Reload player data after augmenting the file
                    player_data = load_player_data_from_csv(CBX_PLAYERS_FILE)
                else:
                    logging.warning("Failed to augment players file; continuing with existing file")
            else:
                logging.warning("Failed to fetch CBX IDs from API; continuing with existing players file")
        else:
        ...
        # Extract CBX IDs from the loaded player data
        cbx_ids = list(player_data.keys())

        print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Processing {len(cbx_ids)} players from file: {CBX_PLAYERS_FILE}\n")
        ...
        print(f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')} - Latest CBX Ratings:\n")
